...
class Crowdflower2 extends \FrameWork {
	...
	private function cfPublish($job, $sandbox) {
		...
		$data = $this->jobConfToCFData ( $jc );
		$csv = $this->batchToCSV ( $job->batch, $job->questionTemplate, $job->extraInfoBatch );
		$gold = $jc->answerfields;
		$options = array (
				"req_ttl_in_seconds" => (isset ( $jc->content ['expirationInMinutes'] ) ? $jc->content ['expirationInMinutes'] : 30) * 60,
		...
		try {
			...
			if (isset ( $id )) {
				...
				$optionsresult = $this->CFJob->setOptions ( $id, array (
						'options' => $options 
				) );
				if ($debug) {
					print "\r\n\r\nOPTIONSRESULT";
				...
				$channelsresult = $this->CFJob->setChannels ( $id, array (
						'cf_internal' 
				) );
				...
				if (is_array ( $gold ) and count ( $gold ) > 0) {
					// TODO: Foreach?
					$goldresult = $this->CFJob->manageGold ( $id, array (
							'check' => $gold [0] 
					) );
				...
				if (! $sandbox and isset ( $csvresult )) {
					$orderresult = $this->CFJob->sendOrder ( $id, count ( $job->batch->parents ), array (
							"cf_internal" 
					) );
				...
				elseif (isset ( $result ['http_code'] )) {
					if ($result ['http_code'] == 503 or $result ['http_code'] == 504)
						$msg = 'Crowdflower service is unavailable, possibly down for maintenance?';
					else
						$msg = "Error creating job on Crowdflower. HTTP code {$result['http_code']}";
				} elseif (Config::get ( 'crowdflower::apikey' ) == '') // empty(method) is not allowed in PHP <5.5
					$msg = 'Crowdflower API key not set. Please check the manual.';
				else
					$msg = "Invalid response from Crowdflower. Is the API down? <a href='http://www.crowdflower.com' target='_blank'>(link)</a>";
